...
func initGlobalRepository() {
	fm := fact2.NewNativeMapRepository()
	fact2.ReplaceGlobals(fm)
	_, err := fact2.R().Create(engine.Fact{Name: "test1", Model: "model1"})
	if err != nil {
		return
	}
	_, err = fact2.R().Create(engine.Fact{Name: "test2", Model: "model2"})
	if err != nil {
		return
	}
}
