...
GO_PACKAGE ?= go list ./... | \
	grep github.com/myrteametrics/myrtea-engine-api/v5/ | \
	grep -v -e "github.com/myrteametrics/myrtea-engine-api/v5/docs" | \
	grep -v -e "github.com/myrteametrics/myrtea-engine-api/v5/internals/tests"
...
test-integration-lw-package:
	GO111MODULE=$(GO111MODULE) GOSUMDB=$(GOSUMDB) CGO_ENABLED=0 go test -p=1 github.com/myrteametrics/myrtea-engine-api/v5/internals/$(GO_PACKAGE)
...
test-integration-package:
	mkdir -p coverage
	GO111MODULE=$(GO111MODULE) GOSUMDB=$(GOSUMDB) CGO_ENABLED=0 go test -p=1 -cover -coverpkg=github.com/myrteametrics/myrtea-engine-api/v5/internals/$(GO_PACKAGE) -coverprofile=coverage/profile.out github.com/myrteametrics/myrtea-engine-api/v5/internals/$(GO_PACKAGE)
	go tool cover -html=coverage/profile.out -o coverage/coverage.html
	go tool cover -func=coverage/profile.out -o coverage/coverage.txt
	cat coverage/coverage.txt
...
.PHONY: security
security:
	TOKEN=$(curl -s -X POST -d '{"login":"admin","password":"myrtea"}' http://localhost:9000/api/v5/login | jq -r '.token')
	curl -X GET -H "Authorization: Bearer zzz$(TOKEN)" http://localhost:9000/api/v5/engine/facts
	curl -X GET -H "Authorization: Bearer $(TOKEN)" http://localhost:9000/api/v5/engine/facts
