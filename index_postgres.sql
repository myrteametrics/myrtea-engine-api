...
drop index if exists idx_situation_history_v4_ts; 
drop index if exists idx_situation_history_v4_situation_id; 
drop index if exists idx_situation_history_v4_situation_instance_id; 
drop index if exists idx_situation_history_v4_combo;
create index idx_situation_history_v4_ts on situation_history_v4 (ts);
create index idx_situation_history_v4_situation_id on situation_history_v4 (situation_id);
create index idx_situation_history_v4_situation_instance_id on situation_history_v4 (situation_instance_id);
create index idx_situation_history_v4_combo on situation_history_v4 (situation_id, situation_instance_id, ts desc nulls last);


explain analyze select distinct on (s.situation_id, s.situation_instance_id) id from situation_history_v4 s order by s.situation_id, s.situation_instance_id, s.ts desc;


drop index if exists idx_situation_history_v4_id; 
drop index if exists idx_fact_history_v4_id; 
drop index if exists idx_situation_fact_history_v4_combo; 
drop index if exists idx_situation_fact_history_v4_situation_history_id;
drop index if exists idx_situation_fact_history_v4_fact_history_id;
create index idx_situation_history_v4_id on situation_history_v4 (id);
create index idx_fact_history_v4_id on fact_history_v4 (id);

create index idx_fact_history_v4_fact_id on fact_history_v4 (fact_id);
create index idx_situation_fact_history_v4_situation_history_id on situation_fact_history_v4 (situation_history_id);
create index idx_situation_fact_history_v4_fact_history_id on situation_fact_history_v4 (fact_history_id);
create index idx_situation_fact_history_v4_combo on situation_fact_history_v4 (situation_history_id, fact_history_id);


explain analyze select * 
from situation_history_v4 s
inner join situation_fact_history_v4 sf on s.id = sf.situation_history_id
inner join fact_history_v4 f on sf.fact_history_id = f.id
where s.situation_id = 3;

explain analyze select * from situation_history_v4 where situation_id = 3 and ts > '2022-08-01';
explain analyze select distinct on (s.situation_id) s.situation_id, ts from situation_history_v4 s order by s.situation_id, s.ts desc;
explain analyze select distinct on (s.situation_id, s.situation_instance_id) s.situation_id, s.situation_instance_id, ts from situation_history_v4 s order by s.situation_id, s.situation_instance_id, s.ts desc;
explain analyze select distinct on (s.situation_id, s.situation_instance_id) * from situation_history_v4 s order by s.situation_id, s.situation_instance_id, s.ts desc;
explain analyze select distinct on (s.situation_id, s.situation_instance_id) * from situation_history_v4 s where ts > now() - '14 days'::interval order by s.situation_id, s.situation_instance_id, s.ts desc;
explain analyze select distinct on (s.situation_id, s.situation_instance_id) id from situation_history_v4 s order by s.situation_id, s.situation_instance_id, s.ts desc;
explain analyze select * from situation_history_v4 where id in (select distinct on (s.situation_id, s.situation_instance_id) id from situation_history_v4 s order by s.situation_id, s.situation_instance_id, s.ts desc);



explain select * from situation_history_v4 s where s.id in (select distinct on (s.situation_id, s.situation_instance_id) id from situation_history_v4 s order by s.situation_id, s.situation_instance_id, s.ts desc);
explain select * from situation_history_v4 s inner join situation_fact_history_v4 sf on s.id = sf.situation_history_id inner join fact_history_v4 f on sf.fact_history_id = f.id ;
explain select * from situation_history_v4 s inner join situation_fact_history_v4 sf on s.id = sf.situation_history_id inner join fact_history_v4 f on sf.fact_history_id = f.id where s.situation_id = 3;
explain select f.* from fact_history_v4 f inner join situation_fact_history_v4 sf on f.id = sf.fact_history_id where sf.situation_history_id in (select distinct on (s.situation_id, s.situation_instance_id) id from situation_history_v4 s order by s.situation_id, s.situation_instance_id, s.ts desc);
...
drop index if exists idx_situation_history_v4_combo;
create index idx_situation_history_v4_combo on situation_history_v4 (situation_id, situation_instance_id, ts desc);
explain analyze select distinct on (s.situation_id, s.situation_instance_id) s.situation_id, s.situation_instance_id, ts from situation_history_v4 s order by s.situation_id, s.situation_instance_id, s.ts desc;

drop index if exists idx_situation_history_v4_combo;
create index idx_situation_history_v4_combo on situation_history_v4 (situation_id, situation_instance_id, ts desc) include (id);
explain analyze select distinct on (s.situation_id, s.situation_instance_id) id from situation_history_v4 s order by s.situation_id, s.situation_instance_id, s.ts desc;



drop index if exists idx_fact_history_v4_ts; 
drop index if exists idx_fact_history_v4_fact_id; 
drop index if exists idx_fact_history_v4_combo; 
create index idx_fact_history_v4_ts on fact_history_v4 (ts);
create index idx_fact_history_v4_fact_id on fact_history_v4 (fact_id);
create index idx_fact_history_v4_combo on fact_history_v4 (fact_id, ts desc) include (id);
explain analyze select distinct on (f.fact_id) id from fact_history_v4 f order by f.fact_id, f.ts desc;
...
from situation_history_v4 s order by s.situation_id, s.situation_instance_id, CAST('2022-08-01' AS TIMESTAMP) + INTERVAL '1 second' * 86400 * FLOOR(DATE_PART('epoch', ts- '2022-08-01')/86400) desc;
...
from situation_history_v4 s order by s.situation_id, s.situation_instance_id, date_trunc('hour', ts) desc;
...
inner join situation_history_v4 sh on (
    sd.id = sh.situation_id 
    and (sh.situation_instance_id = si.id OR sh.situation_instance_id = 0)
)
where sh.id in (select distinct on (s.situation_id, s.situation_instance_id) id from situation_history_v4 s order by s.situation_id, s.situation_instance_id, s.ts desc);
...
// func TestQuery2(t *testing.T) {
    ...
    func TestQuery(t *testing.T) {
        ...
        sqlHistoryFactsDetailsSub, vars, _ := psql.Select("fact_history_id").From("situation_fact_history_v4").Where(sq.Eq{"situation_history_id": ids}).ToSql()
        t.Log(sqlHistoryFactsDetailsSub)
        t.Log(vars)
        // select fact_history_id from situation_fact_history_v4 sf where sf.situation_history_id in (1, 2)
        sqlHistoryFactsDetails, _, err := psql.
            Select("*").
            From("fact_history_v4").
            Where("id in ("+sqlHistoryFactsDetailsSub+")", ids...).
            // Where("id in (select fact_history_id from situation_fact_history_v4 sf where sf.situation_history_id in (" + sqlLastSituation + "))").
            // Where("id in (select fact_history_id from situation_fact_history_v4 sf where sf.situation_history_id in (" + sqlSituationByStandardInterval+ "))").
            // Where("id in (select fact_history_id from situation_fact_history_v4 sf where sf.situation_history_id in (" + sqlSituationByCustomInterval + "))").
            ToSql()
        if err != nil {
            t.Log(err)
        }
