...
func (export StreamedExport) StreamedExportFactHitsFull(ctx context.Context, f engine.Fact, limit int64, factParameters map[string]string) error {
	...
	// handle pit creation
	pit, err := elasticsearch.C().OpenPointInTime(indicesStr).KeepAlive("5m").Do(context.Background())
	if err != nil {
...
// ExportFactHitsFull export data from ElasticSearch and returns a slice of Hits
