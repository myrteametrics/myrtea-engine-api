...
func NewLogicalIndexCronV6(instanceName string, model modeler.Model) (*LogicalIndexCronV6, error) {
	...
	zap.L().Info("Initialize logicalIndex (LogicalIndexCronV6)", zap.String("name", logicalIndexName), zap.String("model", model.Name), zap.Any("options", model.ElasticsearchOptions))

	if model.ElasticsearchOptions.Rollmode != "cron" {
		return nil, errors.New("invalid rollmode for this logicalIndex type")
	}

	logicalIndex := &LogicalIndexCronV6{
		Initialized: false,
...
func (logicalIndex *LogicalIndexCronV6) rollover() {
	ctx := context.Background()
...
func (logicalIndex *LogicalIndexCronV6) persistTechnicalIndex(newIndex string, t time.Time) error {
	if postgres.DB() == nil {
...
func (logicalIndex *LogicalIndexCronV6) purgeTechnicalIndex(technicalIndex string) error {
	if postgres.DB() == nil {
		return errors.New("postgresql Client not initialized")
	}
